...
def flatten_near_earth_object(object: dict):
    ...
    flat_object.update({
        ...
        "miss_distance_kilometers": close_approach_data["miss_distance"]["kilometers"],
        "orbiting_body": close_approach_data["orbiting_body"],
        "relative_velocity_kilometers_per_hour": close_approach_data["relative_velocity"]["kilometers_per_hour"],
    })

    estimated_diameter = object["estimated_diameter"]["kilometers"]
    flat_object.update({
        "estimated_diameter_max_kilometers": estimated_diameter["estimated_diameter_max"],
        "estimated_diameter_min_kilometers": estimated_diameter["estimated_diameter_min"],
    })
...
def datalake_listener(bucket: str, key: str):
    ...
    if hazardous_objects:
        writer = csv.DictWriter(open("hazardous_objects.csv", "w"), flattened_objs[0].keys())
        writer.writeheader()
        writer.writerows(flattened_objs)

        s3_bucket_block.upload_from_path("hazardous_objects.csv", f"{date}_hazardous_objects_by_date/hazardous_objects.csv")
