...
def get_size_change(unprocessed_block_count: int, current_databrics_resouce: str):
    ...
    if unprocessed_block_count < 50 and current_databrics_resouce != 'small':
        logger.info(f"SIZE CHANGE NEEDED: Changing to a SMALLER resource to accomodate {unprocessed_block_count} unprocessed blocks.")
        return 'small'
    elif unprocessed_block_count >= 50 and current_databrics_resouce != 'large':
        logger.info(f"SIZE CHANGE NEEDED: Changing to a LARGER resource to accomodate {unprocessed_block_count} unprocessed blocks.")
        return 'large'
    else:
        logger.info(f"NO SIZE CHANGE NEEDED for {unprocessed_block_count} unprocessed blocks.")
        return None
...
def scale_to(size_change): # prepare job
    ...
    shell_run_command(
        command="echo HI")#"git clone {repo}")
    shell_run_command(
        command="echo BYE")#"cd /repo && gradle build .")
...
def real_time_flow():
    ...
    size_change = get_size_change(
        unprocessed_block_count, 
        current_databricks_resouce)
