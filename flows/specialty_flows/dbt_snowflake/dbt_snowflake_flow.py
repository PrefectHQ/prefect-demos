...
from prefect_dbt.cloud.jobs import trigger_dbt_cloud_job_run_and_wait_for_completion as trigger_dbt_cloud_job_run_and_wait
...
def dbt_flow(fail_on: str = None):
    ...
    run_result_a = trigger_dbt_cloud_job_run_and_wait.with_options(name="Run DBT Source A")(
    ...
    if fail_on == 'dbt_job':
        try:
            run_result_c = trigger_dbt_cloud_job_run_and_wait.with_options(name="DBT Run C")(
                dbt_cloud_credentials=DbtCloudCredentials.load('dbt-creds'),
                job_id=142919,
                max_wait_seconds=timeout
                )
    ...
    check = great_expectations_check.submit(fail_on, wait_for=[run_result_a, run_result_b])
